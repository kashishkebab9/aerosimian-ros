...
public:
  explicit AeroSimianPhiStateNode(const rclcpp::NodeOptions & options)
  : rclcpp::Node("aerosimian_full_state_node", options),
    controller_(make_controller_options(options))
  {
    ...
    RCLCPP_INFO(this->get_logger(), "AeroSimianPhiStateNode component started");
  }
...
private:
  ...
  static mjbots::moteus::Controller::Options make_controller_options(
    ...
    // You can expose this as a parameter if you want:
    // (uncomment and use node options + parameter overrides if needed)
    // int id = 1;
    // if (auto node_base = options.context()) { ... }
...
RCLCPP_COMPONENTS_REGISTER_NODE(AeroSimianPhiStateNode)
